		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'checkbox':
					...
					$output .= '<label class="tlf_ios_cb tlf_checkbox"><input type="checkbox" class="'.$fold.'checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($smof_data[$value['id']], 1, false) .' /><i></i></label>';
				break;
				...
				case 'multicheck': 			
					...
					foreach ($value['options'] as $key => $option) {
						...
						$output .= '<label class="tlf_ios_cb"><input type="checkbox" class="checkbox of-input" name="'.$value['id'].'['.$key.']'.'" id="'. $of_key_string .'" value="1" '. checked($multi_stored[$key], 1, false) .' /><i></i></label><label class="multicheck" for="'. $of_key_string .'">'. $option .'</label><br />';								
					}			 
				break;
				...
				case 'heading':
					...
					$iiii++;
					$jquery_click_hook = "of-option-a".$iiii;
					if ($iiii == 1){
						$curforar = " current";
						$curforartab = ' style="display: block;"';
					}else {
						$curforar = "";
						$curforartab = '';
					}
					$menu .= '<li class="'. $jquery_click_hook .$curforar.'"><a title="'.  $value['name'] .'" href="#'.  $jquery_click_hook  .'"'. $icon .'>'.  $value['name'] .'</a></li>';
					$output .= '<div class="group" id="'. $jquery_click_hook  .'"'.$curforartab.'><h2>'.$value['name'].'</h2>'."\n";
				break;
				...
				case 'backup':
					...
					$output .= '<a href="#" id="of_backup_button" class="button2" title="Backup Options">Backup Options</a>';
					$output .= '<a href="#" id="of_restore_button" class="button2" title="Restore Options">Restore Options</a>';
					$output .= '</div>';
